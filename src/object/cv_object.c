...
int cv_object_has_prop(cv_object_t *obj, int attr, int type) { return cv_object_prop_find(obj->_def->props, attr, type) != NULL; }

int cv_object_prop_get_value(cv_object_t *obj, int prop, int type, cv_variant_t *value) {
  if (!obj) {
    obj = cv_object_global_attrs_get_ptr();
  }
  cv_object_prop_t *prop_ = cv_object_prop_find(obj->_def->props, prop, type);
  if (prop) {
